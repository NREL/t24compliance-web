cbecc.controller('ComplianceCtrl', ['$scope', '$log', '$http', '$timeout', 'toaster', 'data', 'Shared', 'simulation', function ($scope, $log, $http, $timeout, toaster, data, Shared, simulation) {
  ...
  $scope.getXml = function () {
    ...
    $log.debug(params);
    data.bulkSync('simulations', params).then(function(response) {
      $scope.poll();
    }, function(response) {
      toaster.pop('error', 'An error occurred while starting the simulation');
      $log.error('Error starting simulation', response);
    });
  };
