class ZoneSystemsController < ApplicationController
  ...
  def bulk_sync
    ...
    if clean_params.key?('data')
      clean_params[:data].each do |rec|
        ...
        if !cool.nil? && cool.key?('id') && !cool['id'].nil?
        ...
        if !heat.nil? && heat.key?('id') && !heat['id'].nil?
  ...
  # for update_all
  def systems_params
    params.permit(:project_id, :building_id, data: [:id, :name, :building_id, :status, :type, :sub_type, :control_zone_reference, :fan_control, :cooling_control, :count, :cooling_design_supply_air_temperature, :heating_design_supply_air_temperature, :exhaust_system_type, :exhaust_operation_mode, :exhaust_control_method, :hvac_auto_sizing, :description, :air_distribution_type, fan: [:id, :name, :classification, :centrifugal_type, :modeling_method, :control_method, :total_static_pressure, :flow_efficiency, :flow_capacity, :flow_minimum, :motor_bhp, :motor_hp, :motor_type, :motor_pole_count, :motor_efficiency, :motor_position], coil_cooling: [:id, :name, :type, :condenser_type, :dxeer], coil_heating: [:id, :name, :type, :fluid_segment_in_reference, :fluid_segment_out_reference, :fuel_source, :furnace_afue]])
  end
end
