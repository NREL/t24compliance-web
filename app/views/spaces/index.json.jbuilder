json.array!(@spaces) do |space|
  ...
  json.interior_walls space.interior_walls do |interior_wall|
    json.(interior_wall, :id, :name, :area, :adjacent_space_reference, :construct_assembly_reference)
  ...
  json.exterior_walls space.exterior_walls do |exterior_wall|
    ...
    json.windows exterior_wall.windows do |window|
      json.(window, :id, :name, :area, :fenestration_construction_reference)
      json.set! :type, 'Window'
    end
  end

  json.underground_walls space.underground_walls do |underground_wall|
    json.(underground_wall, :id, :name, :area, :height, :construct_assembly_reference, :interior_solar_absorptance, :interior_visible_absorptance,:interior_thermal_absorptance)
    json.set! :surface_type, 'Underground Wall'
  end

  json.underground_floors space.underground_floors do |underground_floor|
    json.(underground_floor, :id, :name, :area, :perimeter_exposed, :construct_assembly_reference, :interior_solar_absorptance, :interior_visible_absorptance,:interior_thermal_absorptance)
    json.set! :surface_type, 'Underground Floor'
  end

  json.exterior_floors space.exterior_floors do |exterior_floor|
    json.(exterior_floor, :id, :name, :area,:construct_assembly_reference)
    json.set! :surface_type, 'Exterior Floor'
  end

  json.interior_floors space.interior_floors do |interior_floor|
    json.(interior_floor, :id, :name, :area,:construct_assembly_reference, :adjacent_space_reference,:interior_solar_absorptance, :interior_visible_absorptance,:interior_thermal_absorptance )
    json.set! :surface_type, 'Interior Floor'
  end

  json.roofs space.roofs do |roof|
    json.(roof, :id, :name, :area, :azimuth, :construct_assembly_reference, :tilt, :interior_solar_absorptance, :interior_thermal_absorptance, :interior_visible_absorptance)
    json.set! :surface_type, 'Roof'
    json.skylights roof.skylights do |skylight|
      json.(skylight, :id, :name, :area, :fenestration_construction_reference)
